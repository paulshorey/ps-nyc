...
require_once($_SERVER['DOCUMENT_ROOT'].'/publisher/core/setup/setup.php');
	...
	// setup
	if ($_GET["sqldate"]) {
		$backupfile = $_SERVER['DOCUMENT_ROOT'].'/backup/data/'.$_GET["sqldate"].'.sql';
	} else {
		$backupfile = $_SERVER['DOCUMENT_ROOT'].'/backup/data/automatic_backup.sql';
	}
	
	// backup
	$connection = @mysql_connect($site["db"]["address"],$site["db"]["username"],$site["db"]["password"]);
	$dumper = new MySQLDump($site["db"]["schema"],$backupfile,false,false);
	$dumper->doDump();
...
?>';*/
	...
	// redirect
	if (!$_SESSION["page"]["message"]){$_SESSION["page"]["message"] = "You've successfully backed up your pages, items, text, and site structure, but not files or images.";}
	redirect_root();
?>
