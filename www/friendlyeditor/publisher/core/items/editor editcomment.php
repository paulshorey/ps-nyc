...
// update
	db_edit('UPDATE '.$_SESSION["site"]["db"]["lang"].'_items SET date="'.$date.'", year="'.$year.'", month="'.$month.'", day="'.$day.'", text1="'.$text1.'", text2="'.$text2.'", text3="'.$text3.'", link="'.$link.'", image1="'.$new_image[0].'", image2="'.$new_image[1].'", image3="'.$new_image[2].'", image4="'.$new_image[3].'", image5="'.$new_image[4].'", video1="'.$new_video[0].'", video2="'.$new_video[1].'", video3="'.$new_video[2].'", video4="'.$new_video[3].'", video5="'.$new_video[4].'" WHERE id='.$id.'');

// send
	// message
	$_SESSION["page"]["message"] = 'You\'ve successfully edited the comment!';
	// page
	redirect_back();
