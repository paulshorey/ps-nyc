	...
	*/
	return function (config) {
		...
		me.setPaused = function (val) {
			...
			document.getElementById('options-container').style.display = ( val ? 'block' : 'none');
		};
		...
		/*
		    ...
		    Direction explained (0 = up, etc etc)
		          ...
		          3   1
		            2
		*/
		me.playArrowKey = function (keyNum) {
			if (isDead || isPaused) {
				return;
			}
		...
		/**
		 * Same as me.playArrowKey, but analyzes a user "tap", as for a mobile device, compared to the snake's head, to translate to relative "up,down,left,right" keys
		 * @method playClick
		 * @param {mouseX, mouseY} position of cursor / tap
		 */
		me.playClick = function (mouseX, mouseY) {
			if (isDead || isPaused) {
				return;
			}
			var snakeLength = me.snakeLength;
			var lastMove = moveQueue[0] || currentDirection;
			var snakeX = me.snakeHead.xPos;
			var snakeY = me.snakeHead.yPos;

			// prefer left
			if (Math.abs(mouseX-snakeX) > Math.abs(mouseY-snakeY) && mouseX < snakeX && me.currentDirection != 3 && (lastMove !== 1 || snakeLength === 1)) {
				moveQueue.unshift(3);
			// prefer right
			} else if (Math.abs(mouseX-snakeX) > Math.abs(mouseY-snakeY) && mouseX > snakeX && me.currentDirection != 1 && (lastMove !== 3 || snakeLength === 1)) {
				moveQueue.unshift(1);
			// up
			} else if (mouseY < snakeY && me.currentDirection != 0 && (lastMove !== 2 || snakeLength === 1)) {
				moveQueue.unshift(0);
			// down
			} else if (mouseY > snakeY && me.currentDirection != 2 && (lastMove !== 0 || snakeLength === 1)) {
				moveQueue.unshift(2);
			// left
			} else if (mouseX < snakeX && me.currentDirection != 3 && (lastMove !== 1 || snakeLength === 1)) {
				moveQueue.unshift(3);
			// right
			} else if (mouseX > snakeX && me.currentDirection != 1 && (lastMove !== 3 || snakeLength === 1)) {
				moveQueue.unshift(1);
			}
		...
		me.go = function () {
			...
			me.currentDirection = currentDirection;
	...
	*/
	...
	return function (inputConfig) {
		...
		var me = this,
			...
			// Board components
			elmContainer, elmPlayingField, elmWelcome, elmTryAgain, elmPauseScreen;
		...
		function createBoardElements() {
			...
			elmLengthPanel.className = "current-score";
			elmLengthPanel.innerHTML = "";
			...
			document.getElementById('options-container').appendChild(elmLengthPanel);
		...
		// game setup
		function gameLoadListen() {
			SNAKE.addEventListener(elmContainer, "keyup",gameLoadKeyup, false);
			SNAKE.addEventListener(elmContainer, "click", gameLoad, false);
		};
		function gameLoadKeyup() {
			if (!evt) var evt = window.event;
		...
		function gameLoad() {
			if (me.getBoardState()==0) {
				me.welcomeElm.style.display = "none";
				me.reloadElm.style.display = "none";
				me.resetBoard();
				me.setBoardState(1);
				me.getBoardContainer().focus();
				mySnake.go();
				//
				SNAKE.addEventListener(elmContainer, "keyup", gameKeyup, false);
				SNAKE.addEventListener(elmContainer, "click", gameClick, false);
		...
		function gameClick(e) {
			...
			// state 2 -> 2
				...
				mySnake.playClick(e.offsetX, e.offsetY);
			}
		};

		// game reset
		function createWelcomeElement() {
			me.welcomeElm = document.createElement("div");
			me.welcomeElm.id = "sbWelcome" + myId;
			me.welcomeElm.className = "snake-welcome-dialog";
			...
			me.welcomeElm.appendChild(welcomeTxt);
			me.welcomeElm.appendChild(welcomeStart);
			return me.welcomeElm;
		};
		function createTryAgainElement() {
			me.reloadElm = document.createElement("div");
			me.reloadElm.id = "sbTryAgain" + myId;
			me.reloadElm.className = "snake-try-again-dialog";
			...
			me.reloadElm.appendChild(tryAgainTxt);
			me.reloadElm.appendChild(tryAgainStart);
			return me.reloadElm;
		};
